    ...
    >
        <div class="flex flex-col justify-center">
            <div class="flex flex-col items-center ">
                <div
                    :style="!$store.state.labelModule.printing ? 'margin-bottom: 0' : ($store.state.labelModule.barcodeOptions.fontSize < 10 ? 'margin-bottom: -3px' : 'margin-bottom: -7px')">
            ...
            <div class="flex justify-between h-fit">
                ...
                >
                    <div
                        v-if="$store.state.labelModule.printSettings.eac == true"
                        class="w-[18px]"
                    >
                        <svg
                            viewBox="0 0 500 500"
            ...
            <div
                v-if="$store.state.labelModule.printSettings.noCertificate == true"
                :style="$store.state.labelModule.printSettings.centered ? 'align-items: center;  text-align: center;' : null"
            >Товар не подлежит обязательной сертификации</div>
        </div>
    </div>
</template>
<script>
